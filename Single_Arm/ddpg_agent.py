...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        if Agent.actor_local is None:
            Agent.actor_local = Actor(state_size, action_size, random_seed, Actor_HL1, Actor_HL2).to(device)
        if Agent.actor_target is None:
            Agent.actor_target = Actor(state_size, action_size, random_seed, Actor_HL1, Actor_HL2).to(device)
        ...
        if Agent.critic_local is None:
            Agent.critic_local = Critic(state_size, action_size, random_seed, Critic_HL1, Critic_HL2).to(device)
        if Agent.critic_target is None:
            Agent.critic_target = Critic(state_size, action_size, random_seed, Critic_HL1, Critic_HL2).to(device)
    ...
    def step(self, time_step, state, action, reward, next_state, done):
        ...
        if time_step % N_DELAYED_STEPS != 0:
            return

        # Learn, if enough samples are available in memory
        if len(Agent.memory) > BATCH_SIZE:
            for i in range(N_EXPERIENCE_UPDATES):
